class MaxHeap:
    ...
    def _heapify_down(self, index):
        ...
        target = index
        if left < heap_len and self.heap[target][0] < self.heap[left][0]:
            target = left
        if right < heap_len and self.heap[target][0] < self.heap[right][0]:
            target = right
...
class Twitter:
    ...
    def postTweet(self, userId: int, tweetId: int) -> None:
        if userId not in self.tweets_dict:
            self.tweets_dict[userId] = [(self.tweet_counter, tweetId)]
        else:
            self.tweets_dict[userId].append((self.tweet_counter, tweetId))
        self.tweet_counter += 1
