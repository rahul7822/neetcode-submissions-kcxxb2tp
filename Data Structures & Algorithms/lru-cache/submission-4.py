...
class LRUCache:
    ...
    def get(self, key: int) -> int:
        if key not in self.mapping:
            ...
            if self.cache_ll_tail == self.cache_ll_head:
                return node.val

            if node == self.cache_ll_tail:
                return node.val
            
            if node == self.cache_ll_head:
                self.cache_ll_head = self.cache_ll_head.next
                self.cache_ll_head.prev = None
    ...
    def put(self, key: int, value: int) -> None:
        if len(self.mapping) < self.capacity and key not in self.mapping:
            ...
            self.cache_ll_head.prev = None
            self.cache_ll_tail.next = node
            self.cache_ll_tail = self.cache_ll_tail.next
            self.mapping[key] = node
